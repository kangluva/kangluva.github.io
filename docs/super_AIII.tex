...
Let $L=L(\bla(u))$ be a highest $\ell_\s$-weight module over $\YMN$ with a highest $\ell_\s$-weight vector $\xi$. Let $V=V(\bm\mu(u))$ be a highest $\ell_{\s,\bm\ve}$-weight module over $\BMN$ with a highest $\ell_{\s,\bm\ve}$-weight vector $\eta$. We end this section by showing that $\xi\otimes \eta$ is a highest $\ell_{\s,\bm\ve}$-weight vector and  investigating its highest $\ell_{\s,\bm\ve}$-weight.
...
\begin{align*}
\Delta(b_{ii}(u)) \approx \sum_{a=i}^\ka t_{ia}(u)t_{ai}'(-u)\otimes b_{aa}(u)(\xi\otimes\eta).
\end{align*}
Therefore,
\begin{align*}
...
\[
\tl{b}_{ii}(u)(\xi\otimes\eta_\gamma)=\frac{2\ve_i u-\ve_i\rho_{i+1}+\varpi_{i+1}+2\gamma}{u-\gamma}\la_i(u)\la_{i}'(-u)(\xi\otimes\eta_\gamma),
\]
...
\begin{dfn}
For $\vartheta_1\in\bC$ and $l\in\Z_{>0}$, the dAHA $\mathscr{H}_{\vartheta}^l$ of type $\mathrm A_l$ is the associative algebra generated by the group algebra $\bC[\fkS_l]$ and $y_1,\cdots,y_l$ with the relations $y_iy_j=y_jy_i$, $1\lle i,j\lle l$, and
\begin{align*}
...
\]
\end{dfn}
The following lemmas are known, see e.g. \cite[Section 2]{Chen2014twisted}.
\begin{lem}
...
\varsigma_l\sfy_i=\sfy_{l}\varsigma_i,\qquad i\ne l,
\]
\begin{align*}
...
\end{lem}
\begin{proof}
In the same way of Proposition \ref{thm:embedding}, it suffices to show that the matrix $G_{\ve}+\gamma u^{-1}$ satisfies the reflection equation \eqref{eq:reflectG} which it is known in \cite{Arnaudon2004general,Ragoucy2007analytical,Belliard2009nested}.
\end{proof}
...
Let $l,l_1,l_2\in\bZ$ such that $l=l_1+l_2$. Let $M_1$ be an $\mathscr H_{\vartheta_1}^{l_1}$-module, $M_2$ an $\mathscr H_{\vartheta_1,\vartheta_2}^{l_2}$-module. Set
\[
...
\]
see \eqref{eq:embedding-hecke}. Then $M_1\odot M_2$ is an $\mathscr H_{\vartheta_1,\vartheta_2}^{l}$-module and hence $\mc D_{\s,\bm e}^\ve(M_1\odot M_2)$ is $\BMN$-module.
...
We set
$$
\mc B^{\chi}(u)=\sum_{r\in \bZ_{\gge 0}}\sum_{i,j=1}^\ka \sfb_{ij}^{(r)}u^{-r}\otimes E_{ij},
$$
